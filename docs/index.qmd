...
format: 
  revealjs:
    ...
    footer: "njtierney.github.io/conmat-tki-2024/"
editor: visual
execute: 
  echo: false
  warning: false
...
-   Discuss **other applications** of contact matrices outside of COVID
...
-   2020 - 2024: **Research Software Engineer** \@ Telethon Kids Institute
...
-   If we know **the number of times people have contact**, we have an idea of **which age groups** might spread COVID
...
-   $X^{L}_{i,\alpha}$ is the number of contacts made by individual $i$ at a particular location $L$ with someone in age group $\alpha$ is modelled to be Poisson with mean $\mu_{i, \alpha}^{L}$.

-   the ages of individual $i$, $\alpha_i$, and their contact, $\alpha$ are broken into 5 year age intervals

-   $L$ is contact location: L = ...Home, Work, School, Other
...
-   [Code is provided](https://github.com/kieshaprem/synthetic-contact-matrices), but a few key issues:
    ...
    -   It was code not written for reuse (code vs software)
        ...
        -   No interface for getting inputs for a given country or region.

        -   Challenging to see which code matches which methods

-   Australia in original 155 countries, but not the updated 177!

## Building our own extension

-   Nick Golding wrote a new method that was able to be more flexible, using Generalised Additive Models (GAMs) instead of Bayesian approach.
...
## Contact matrix: Subiaco

```{r}
#| label: plot-matrix-subiaco
#| echo: true
...
- Could apply to other infectious diseases that socially spread
  - e.g., RSV, Influenza
- Could be used to evaluate impact and understand where to vaccinate.
- Chitra Saraswati has produced already-created CSV files for >180 countries.


## Demonstrating use in a (simple) Susceptible-Infectious-Recovered model



## Future Directions

-   Allow for using smaller age groups (currently limited to 6 monthly ages)
...
`r icon_link` [njtierney.github.io/conmat-tki-2024](njtierney.github.io/conmat-tki-2024)
